...
if isa(query, "yfinance.internal.ScreenerQuery")
    ...
    lookupQueryName = lower(queryName);
    [predefinedQuery, isPredefinedQuery] = yfinance.internal.predefinedScreenerQueryDefinition(lookupQueryName);
    requestQueryName = queryName;

    if isPredefinedQuery
        requestQueryName = lookupQueryName;
    end

    count = predefinedCount(options.Count, options.Size);

    if isPredefinedQuery && options.Offset > 0
        response = options.Session.postScreener( ...
            predefinedQuery.Query.toStruct(), ...
            Count=count, ...
            Offset=options.Offset, ...
            SortField=predefinedQuery.SortField, ...
            SortAscending=predefinedQuery.SortAscending, ...
            QuoteType=predefinedQuery.QuoteType);
    else
    ...
    result = yfinance.internal.screenerResponseToResult(response, Query=requestQueryName);
end
end
